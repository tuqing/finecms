...
class CI_Router {
	...
	protected function _set_routing()
	{
		...
		if (file_exists(APPPATH.'config/routes.php')) {
			...
			if (file_exists(APPPATH.'config/rewrite.php')) include(APPPATH.'config/rewrite.php');
		} else {
			include(FCPATH.'config/routes.php');
			if (file_exists(FCPATH.'config/rewrite.php')) include(FCPATH.'config/rewrite.php');
		}
		...
		if (defined('DR_URI')) {
			...
			// 没有找到返回404
			if (!$uri) {
				if (!is_file(FCPATH.'cache/install.lock') && is_dir($_SERVER['DOCUMENT_ROOT'].'/'.DR_URI)) {
					show_error('FineCMS禁止子目录安装，请放置在网站根目录安装', 404);
				} else {
					show_error('FineCMS无法找到对应的页面('.DR_URI.'),可能您没有配置好路由规则', 404);
				}
			}
	...
	public function set_class($class)
	{
		...
		$this->class = $this->class ? $this->class : 'home';
	}
