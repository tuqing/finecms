...
if ( ! function_exists('get_config'))
{
	...
	function &get_config(Array $replace = array())
	{
		...
		if (empty($_config))
		{
			...
			if (file_exists($file_path = ENVIRONMENT.'/config.php'))
			{
				require($file_path);
			}
			elseif ( ! $found)
...
if ( ! function_exists('get_mimes'))
{
	...
	function &get_mimes()
	{
		...
		if (file_exists(ENVIRONMENT.'/mimes.php'))
		{
			$_mimes = include(ENVIRONMENT.'/mimes.php');
		}
...
if ( ! function_exists('set_status_header'))
{
	...
	function set_status_header($code = 200, $text = '')
	{
		...
		if (empty($text))
		{
			...
			{
				show_error('没有可用的状态文本，请检查您的状态码数', 500);
			}
		}
