...
class CI_Loader {
	...
	public function helper($helpers = array())
	{
		foreach ($this->_ci_prep_filename($helpers, '_helper') as $helper)
		{
			...
			if ( ! isset($this->_ci_helpers[$helper]))
			{
				show_error('无法加载文件: helpers/'.$helper.'.php');
			}
		}
	}
	...
	public function add_package_path($path, $view_cascade = TRUE)
	{
		...
		$this->_ci_library_paths = array_unique($this->_ci_library_paths); // 去除重复项
		$this->_ci_model_paths = array_unique($this->_ci_model_paths); // 去除重复项
		$this->_ci_helper_paths = array_unique($this->_ci_helper_paths); // 去除重复项
		...
		$config->_config_paths = array_unique($config->_config_paths); // 去除重复项
	}
	...
	protected function _ci_load_class($class, $params = NULL, $object_name = NULL)
	{
		...
		if (file_exists($subclass))
		{
			...
			{
				log_message('error', '无法加载所需的类: '.$class);
				show_error('无法加载所需的类: '.$class);
			}
	...
	protected function _ci_init_class($class, $prefix = '', $config = FALSE, $object_name = NULL)
	{
		...
		if ($config === NULL)
		{
			...
			if (is_array($config_component->_config_paths))
			{
				...
				foreach ($config_component->_config_paths as $path)
				{
					...
					// first, global next
					if (file_exists(ENVIRONMENT.'/'.strtolower($class).'.php'))
					{
						include(ENVIRONMENT.'/'.strtolower($class).'.php');
						break;
					}
					elseif (file_exists(ENVIRONMENT.'/'.ucfirst(strtolower($class)).'.php'))
					{
						include(ENVIRONMENT.'/'.ucfirst(strtolower($class)).'.php');
						break;
					}
		...
		if (isset($CI->$object_name))
		{
			...
			show_error("资源 '".$object_name."' 已经存在，".$class." 不是一个实例");
		}
	...
	{
		if (file_exists(ENVIRONMENT.'/autoload.php'))
		{
