...
class CI_Image_lib {
	 ...
	 * @return	bool
	 */
	public function resize($thumb_file = NULL)
	{
	 ...
	 * @return	bool
	 */
	public function watermark($src_img = NULL)
	{
		return ($this->wm_type === 'overlay') ? $this->overlay_watermark($src_img) : $this->text_watermark($src_img);
	}
	 ...
	 * @return	bool
	 */
	public function overlay_watermark($src_img = NULL)
	{
		...
		// 修正图片宽高
		if ($src_img && $this->is_watermark === TRUE) {
			$this->orig_width	= $this->width;
			$this->orig_height	= $this->height;
		}
		// Create two image resources
		$wm_img  = $this->image_create_gd($this->wm_overlay_path, $wm_img_type);
		$src_img = $src_img && $this->is_watermark === TRUE ? $src_img : $this->image_create_gd($this->full_src_path);
	 ...
	 * @return	bool
	 */
	public function text_watermark($src_img = NULL)
	{
		$src_img = $src_img && $this->is_watermark === TRUE ? $src_img : $this->image_create_gd();
		if ( ! $src_img)
		{
			return FALSE;
		}
	...
	public function image_display_gd($resource)
	{
		...
		switch ($this->image_type)
		{
			case 1	:	
				imagegif($resource);
				if ($this->thumb_file) imagegif($resource, $this->thumb_file);
				break;
			case 2	:	
				imagejpeg($resource, NULL, $this->quality);
				if ($this->thumb_file) imagejpeg($resource, $this->thumb_file, $this->quality);
				break;
			case 3	:	
				imagepng($resource);
				if ($this->thumb_file) imagepng($resource, $this->thumb_file);
				break;
